...
/*
 * Updated by Omnisense Ltd to support the geo location application
 * November 2016
 */
...
uint8_t MPL3115A2::setInterrupts(INT1_CONFIGURATION iconfig1, INT2_CONFIGURATION iconfig2,
                INT_MAPPING intmap, INT_ENABLES intenable)
{
    ...
    result |= MPL3115A2::readRegister(CTRL_REG3, datain, 3);
    dataout[0] = (datain[0] & 0x33) | iconfig1 | iconfig2;
    dataout[1] = intenable;
    dataout[2] = intmap;  
    result |= MPL3115A2::writeRegister(CTRL_REG3, dataout, 3);
...
//    debug("Sending to %x : %x, %d bytes, first %x\n",_i2c_addr,reg,count,buf[1]);
